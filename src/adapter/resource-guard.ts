...
export class ResourceGuard {
  ...
  constructor(
    memoryCeilingPct: number = 90,
    maxWorkers: number = 4,
    getActiveWorkerCount: () => number = () => 0,
    cpuCeilingPct: number = 90,
  ) {
   ...
   *
   * Uses hysteresis to avoid oscillation: recovery only fires when usage
   * drops 15% below the ceiling (e.g., ceiling 90% → recovery at 75%).
