...
// Mock os module — CPU mocking is needed for deterministic CPU% tests.
// Memory reads real system values via vm_stat on macOS, so tests that
// expect "healthy" use high ceilings (99%) and tests that expect
// "unhealthy" use low ceilings (5%) to work on any machine.
vi.mock("node:os", () => ({
  freemem: vi.fn(() => 8 * 1024 * 1024 * 1024), // fallback if vm_stat fails
...
// Ceilings matching production defaults (90% memory/CPU).
// Tests expect real system memory to be below 90% — same as production.
const HIGH_CEIL = 90;
const LOW_CEIL = 5;   // always unhealthy (real memory is always >5%)

describe("ResourceGuard", () => {
  describe("check()", () => {
    it("should return healthy when memory is below ceiling", () => {
      const guard = new ResourceGuard(HIGH_CEIL, 4, () => 0);
      const snap = guard.check();
      expect(snap.healthy).toBe(true);
      expect(snap.memoryUsedPct).toBeGreaterThanOrEqual(0);
      expect(snap.memoryUsedPct).toBeLessThan(HIGH_CEIL);
    ...
    it("should return correct worker counts", () => {
      let count = 0;
      const guard = new ResourceGuard(HIGH_CEIL, 8, () => count);
  ...
  describe("userFacingStatus()", () => {
    ...
    it("should include both memory and worker warnings when both constrained", () => {
      ...
      const guard = new ResourceGuard(LOW_CEIL, 2, () => 2);
      const status = guard.userFacingStatus();
      expect(status).not.toBeNull();
      expect(status).toContain("Memory usage is high");
      expect(status).toContain("All worker slots are in use");
  ...
  describe("CPU monitoring", () => {
    ...
    it("should report CPU healthy when usage is below ceiling", () => {
      // CPU = 0% (constant mock), default ceiling 90% → healthy
      const guard = new ResourceGuard(HIGH_CEIL, 4, () => 0);
      const snap = guard.check();
    ...
    it("should report CPU unhealthy when usage exceeds ceiling", () => {
      ...
      // cpuCeilingPct = 90, so 90% >= 90% → unhealthy
      const guard = new ResourceGuard(HIGH_CEIL, 4, () => 0, HIGH_CEIL);
      const snap = guard.check();
    ...
    it("should block canSpawnMore when CPU is over ceiling even with memory and workers OK", () => {
      ...
      // Memory ceiling 90% (real system → OK), workers 0/4 → OK, CPU 90% > 50% ceiling → blocked
      const guard = new ResourceGuard(HIGH_CEIL, 4, () => 0, 50);
      const snap = guard.check();
    ...
    it("should mention CPU in userFacingStatus when CPU is the issue", () => {
      ...
      // Memory OK (90% ceiling), CPU 90% > 50% ceiling
      const guard = new ResourceGuard(HIGH_CEIL, 4, () => 0, 50);
      const status = guard.userFacingStatus();
    ...
    it("should mention only memory in userFacingStatus when only memory is the issue", () => {
      // CPU = 0% (constant mock), CPU ceiling 90% → CPU OK
      // Memory ~50%, memory ceiling 40% → memory NOT OK
      const guard = new ResourceGuard(LOW_CEIL, 4, () => 0, HIGH_CEIL);
      const status = guard.userFacingStatus();
  ...
  describe("startMonitoring() / stopMonitoring()", () => {
    ...
    it("should fire recovery only when usage drops 15% below ceiling (hysteresis)", () => {
      let workers = 4;
      // Use 99% ceilings so real system memory/CPU never blocks recovery
    ...
    it("should NOT fire recovery if memory is still above recovery threshold (hysteresis blocks)", () => {
      let workers = 4;
      const guard = new ResourceGuard(LOW_CEIL, 4, () => workers);
      const transitions: ResourceTransition[] = [];
      guard.startMonitoring((t) => transitions.push(t), 1000);
      vi.advanceTimersByTime(1000);
      expect(transitions).toHaveLength(1);
