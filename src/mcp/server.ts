...
/**
 * Validate path arguments in file-reading commands against BLOCKED_PREFIXES.
 * Resolves relative paths against the command's cwd (not process.cwd()),
 * and follows symlinks via realpath to prevent bypass.
 */
const FILE_READING_COMMANDS = new Set(["cat", "head", "tail", "ls", "find", "wc", "tree"]);

async function validatePathArgs(
  exe: string, args: string[], cwd: string | undefined,
): Promise<string | null> {
  if (!FILE_READING_COMMANDS.has(exe)) return null;

  for (const arg of args) {
    // Extract path candidates from the argument:
    // - bare paths: "/etc/passwd", "relative/path"
    // - flags with =value: "--gitfile=/etc/passwd"
    // - pure flags without paths: "-n", "--help" → skip
    let pathCandidate: string | null = null;
    if (!arg.startsWith("-")) {
      pathCandidate = arg;
    } else if (arg.includes("=")) {
      pathCandidate = arg.split("=").slice(1).join("="); // value after first =
    }
    // Skip pure flags (no path content)
    if (!pathCandidate || pathCandidate.length === 0) continue;

    // Resolve relative to the command's cwd, not process.cwd()
    const resolved = isAbsolute(pathCandidate) ? pathCandidate : resolve(cwd ?? process.cwd(), pathCandidate);
    // Follow symlinks to prevent symlink-based bypass
    let real: string;
    try {
      real = await realpath(resolved);
    } catch {
      real = resolved; // file may not exist yet — check the resolved path
    }
    for (const prefix of BLOCKED_PREFIXES) {
...
server.tool(
  ...
  async (args) => {
    ...
    // Block path arguments to file-reading commands (resolves relative to cwd, follows symlinks)
    const pathError = await validatePathArgs(parsed.exe, parsed.args, cwd);
    if (pathError) {
      return { content: [{ type: "text" as const, text: `Error: ${pathError}` }], isError: true };
    }
