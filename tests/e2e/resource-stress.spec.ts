...
test.beforeEach(async ({ page }) => {
  ...
  // If we land on the login page, auth setup hasn't been run
  if (page.url().includes("/login")) {
    throw new Error(
      "Discord auth not set up. Run: npm run test:e2e:setup\n" +
      "See playwright.config.ts for full instructions.",
    );
  }
