...
> **WARNING: This skill is designed for use inside  running in a VM only.**
>  must NEVER run on the host OS. The runtime guard blocks  execution unless `_VM_CONFIRMED=1` is set — this variable must only exist inside a dedicated VM. See SETUP.md Step 6 and DECISIONS.md for full rationale. Running  on a host machine is dangerous — it installs persistent daemon processes with broad filesystem/network access.
